...
#### A. 画布层 (The Canvas)
*   **背景**: **极简纯净**。建议仅保留极低透明度 (e.g. 3-5%) 的微弱点阵，仅用于提供移动时的视觉参照 (防止迷失)，而非强制对齐。亦可考虑仅在拖拽时显示。
*   **导航控制**:
    *   支持 鼠标滚轮缩放 (Zoom)。
    *   支持 空格+拖拽 或 中键平移 (Pan)。
...
*   **设计原则**: 严格限制“无根节点”的产生，强制“生长”隐喻。
*   功能按钮:
    *   Select (选择模式 - 默认)
    *   Hand (漫游模式)
    *   Connect (连线工具 - 快捷键 `L`)。
    *   Layout (自动整理布局)。
    *   *注: 移除了显性的 "Add Node" 按钮。初始节点仅通过双击画布创建，后续节点均由父节点派生，确保逻辑流连贯。*
